...
`test.js` — **1,674 assertions, headless Node, no dependencies**, about 33 seconds.
It prints its own wall clock, and says so when it crosses the 30-second stop
condition, because that was the one stop condition nothing measured.
...
7. **Assertions tagged `REGRESSION` guard a bug that actually shipped**, and each was
   ...
   swap.
8. **The documentation is held to the same standard as the code.** `HANDBOOK.md`
   and `handbook.html` are generated by `tools/handbook.js` out of the live tables,
   never written by hand, and the suite reads the generated Markdown back and
   asserts every number in it against the table it came from. It also runs
   `node tools/handbook.js --check`, which regenerates into memory and fails if
   what is on disk differs. Retune a weapon and forget to regenerate and the build
   is red. **A guide that lies is worse than no guide.**
9. **Exit arrivals are checked twice** — once on the geometry, and once by actually
...
*(Rules 4's cheat clause, 7 and 9 recovered from `bible/archive/11-TECH.md` §11.6 — they
describe the shipped suite and the new set had dropped them.)*

### The handbook generator
`tools/handbook.js` loads the game in a small `vm` sandbox — enough stub to reach
`__ODINALA_TEST`, not enough to drive it — and exposes the live tables plus a set of
constants it reads out of `odinala.html` **by pattern**, throwing if a pattern stops
matching. Enemy stats come off the `base(x,y,w,h,hp,poise)` builders by discovery
rather than a list; tell colours and wind-up lengths come out of each update
function. `tools/handbook-content.js` holds the prose and emits one structured node
array that both the Markdown and the HTML emitter consume.

**No number is typed into either file.** If you find yourself writing a frame count
into the prose, read it off the tables instead — the suite checks the document, not
the generator, so a hand-typed number that happens to be right today drifts silently
tomorrow.
